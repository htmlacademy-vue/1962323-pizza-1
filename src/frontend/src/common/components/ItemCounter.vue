<template>
  <div :class="`counter ${counterClass}`">
    <button
      type="button"
      :class="`counter__button counter__button--minus ${minusClass}`"
      @click="changeCounter('decrement', -1, product.quantity)"
    ...
    <input
      type="text"
      :name="`counter-${product.id}`"
      class="counter__input"
      :value="product.quantity"
      @change="
        changeCounter('input', Number($event.target.value), product.quantity)
      "
    />
    <button
      type="button"
      :class="`counter__button counter__button--plus ${plusClass}`"
      @click="changeCounter('increment', 1, product.quantity)"
