  ...
  <div class="content__pizza">
        ...
        <div :class="`pizza pizza--foundation--${getViewClass}`">
          <div class="pizza__wrapper">
            <transition-group name="fade">
              <div
                v-for="ingredient in choosenIngredients"
                :key="ingredient.id"
              >
                <transition name="fade">
                  <div
                    :class="`pizza__filling pizza__filling--${ingredient.class}`"
                    v-show="ingredient.quantity >= 1"
                  ></div>
                </transition>
                <transition name="fade">
                  <div
                    :class="`pizza__filling pizza__filling--${ingredient.class} pizza__filling--second`"
                    v-show="ingredient.quantity >= 2"
                  ></div>
                </transition>
                <transition name="fade">
                  <div
                    :class="`pizza__filling pizza__filling--${ingredient.class} pizza__filling--third`"
                    v-show="ingredient.quantity >= 3"
                  ></div>
                </transition>
              </div>
            </transition-group>
    ...
    <div class="content__result">
      ...
      <button
        ...
        class="button"
        @click="addToCart"
        :disabled="choosenIngredients.length == 0 || !configuredPizza.name"
      >
        Готовьте!
      </button>
    </div>
...
export default {
  ...
  computed: {
    ...
    getViewClass() {
      ...
      let sauce = this.getPizzaComponent(
        "sauces",
        this.configuredPizza.sauceId
      );
