...
export default {
  ...
  mutations: {
    ...
    [SET_INGREDIENT_COUNT](state, { id, quantity }) {
      ...
      let ingredientIndex = state.configuredPizza.ingredients.findIndex(
  ...
  getters: {
    ...
    ingredients(state) {
      return [...state.ingredients].map((ingredient) => {
        let selectedIngredient = state.configuredPizza.ingredients.find(
          (elem) => elem.ingredientId == ingredient.id
        );
        return {
          ...ingredient,
          quantity: selectedIngredient ? selectedIngredient.quantity : 0,
        };
      });
    },
